...
TIP: SCV를 사용하여 수행한 백업을 보조 스토리지에 복제하는 것 외에도 NetApp Backup and Recovery for VMs를 사용하면 3대 클라우드 제공업체 중 하나의 객체 스토리지에 데이터의 오프사이트 복사본을 만들 수 있습니다. 자세한 내용은 본 자료를 참조하십시오.link:https://docs.netapp.com/us-en/data-services-backup-recovery/index.html["NetApp Backup and Recovery 설명서"] .
