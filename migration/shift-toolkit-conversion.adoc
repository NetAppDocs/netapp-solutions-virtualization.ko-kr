...
NOTE: OLVM의 경우 qcow2 파일 형식도 사용할 수 있습니다. 이러한 선택은 청사진을 만드는 동안 이루어질 수 있습니다.
...
NOTE: VM에서 변환된 RAW 디스크 이미지를 사용하려면 OpenShift 콘솔을 통해 또는 OpenShift의 경우 YAML을 통한 OC 명령을 통해, OLVM의 경우 OLVM REST API/UI를 사용하여 VM을 수동으로 생성하고 디스크를 연결해야 합니다. 이와 함께 네트워크도 수동으로 매핑해야 합니다.

+

NOTE: VM을 수동으로 생성할 때 적절한 부팅 유형(EFI 또는 BIOS)이 지정되었는지 확인하세요.
