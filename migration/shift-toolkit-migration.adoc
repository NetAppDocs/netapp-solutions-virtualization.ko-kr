...
계획된 유지 관리 기간 동안에는 VM의 전원을 정상적으로 꺼야 합니다.  마이그레이션을 시작하기 전에 모든 전제 조건이 충족되었는지 확인하세요.

* Shift Toolkit VM이 도메인의 일부인지 확인하세요.
* CIFS 공유가 적절한 권한으로 구성되었는지 확인하세요.
* 마이그레이션 또는 변환에 사용된 qtree에 올바른 보안 스타일이 있는지 확인하세요.
* 클러스터 내의 모든 Hyper-V 호스트에서 Hyper-V 관리자를 사용하여 VM 생성을 테스트하고 VHDx를 CIFS 공유에 배치합니다.
* 프로그램 및 기능 또는 PowerShell을 사용하여 Hyper-V 관리 도구를 추가하여 Shift Toolkit VM에서 VM 생성을 테스트합니다. `add-windowsfeature rsat-hyper-v-tools`
