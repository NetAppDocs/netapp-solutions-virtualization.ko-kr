...
---
= Trident Protect를 사용하여 Red Hat OpenShift Virtualization에서 VM에 대한 페일오버 및 페일백 설정
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
* OpenShift VM에 대한 페일오버 및 페일백 작업을 구현하려면 Trident Protect를 설치해야 합니다. 설치 방법은 여기의 지침을 참조하십시오link:https://docs.netapp.com/us-en/trident/trident-protect/trident-protect-installation.html["Trident Protect를 설치하십시오"]


image:redhat-openshift-ocpv-tp-001.png["OCP-v Trident Protect가 trident-protect 네임스페이스에 설치되었습니다"]
...
이 섹션에서는 ONTAP S3 객체 스토리지를 사용하여 Trident Protect에서 AppVault 객체를 설정하는 방법을 보여줍니다.

아래에 표시된 oc 명령어와 yaml 파일을 사용하여 ONTAP S3용 시크릿과 AppVault 사용자 지정 리소스를 생성하십시오. 반드시 trident protect 네임스페이스에 생성해야 합니다.
...
== 새로운 네임스페이스에 재해 복구 VM용 Trident Protect 앱을 생성합니다.
