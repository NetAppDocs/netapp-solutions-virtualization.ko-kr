...
---
= Trident Protect를 사용하여 Red Hat OpenShift Virtualization의 VM 보호
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
OpenShift Virtualization VM의 영구 디스크는 link:https://docs.netapp.com/us-en/trident/["Trident CSI"]을 사용하여 OpenShift 클러스터에 통합된 ONTAP 스토리지로 백업할 수 있습니다. 이 섹션에서는 link:https://docs.netapp.com/us-en/trident/trident-protect/learn-about-trident-protect.html["Trident Protect"]를 사용하여 데이터 볼륨을 포함한 VM의 스냅샷 및 백업을 ONTAP 객체 스토리지에 생성합니다.

필요한 경우 스냅샷이나 백업에서 복원합니다.

Trident Protect는 OpenShift 클러스터의 애플리케이션 및 VM에 대한 스냅샷, 백업, 복원 및 재해 복구를 지원합니다. OpenShift Virtualization VM의 경우, Trident Protect로 보호할 수 있는 데이터에는 VM과 연결된 Kubernetes 리소스 객체, 영구 볼륨 및 내부 이미지가 포함됩니다.
...
[%collapsible%open]
====
애플리케이션 또는 VM의 스냅샷 및 백업을 생성하기 전에 Trident Protect에서 스냅샷 및 백업을 저장할 Object Storage를 구성해야 합니다. 이는 bucket CR을 사용하여 수행됩니다. bucket CR은 관리자만 생성하고 구성할 수 있습니다. Trident Protect에서 bucket CR은 AppVault라고 합니다. AppVault 객체는 스토리지 bucket을 Kubernetes 워크플로에서 선언적으로 표현한 것입니다. AppVault CR에는 백업, 스냅샷, 복원 작업 및 SnapMirror 복제와 같은 보호 작업에 bucket을 사용하는 데 필요한 구성이 포함되어 있습니다.
